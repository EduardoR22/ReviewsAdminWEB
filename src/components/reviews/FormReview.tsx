...
import ButtonIssue from "./reviews/ButtonIssue";
import { createReview } from "@/api/routeReviews";
import StarElement from "./reviews/StarElement";
import UploadImage from "./reviews/UploadImage";
import Comments from "./reviews/Comments";
import InputName from "./reviews/InputName";
import InputEmail from "./reviews/InputEmail";
import SelectProjects from "./reviews/SelectProjects";
...
export default function FormReview({issues, projects}: {issues:any, projects:any}){
  ...
  const formik = useFormik({
    ...
    onSubmit: async (valores) => {
      ...
      try {
        ...
        if(res === 201) console.log('exitooo');
      } catch (error) {
        console.log('error');
        console.log(error);
      }                                  
  ...
  useEffect(() => {
    ...
    setFilter(filtered);
    const issuesAux:any[] = [];
    filtered.map((issueFilter:any, index:number) => {
      issuesAux.push({
        id: issueFilter._id,
        state: false,
      })
